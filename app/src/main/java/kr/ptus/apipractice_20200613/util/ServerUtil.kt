...
//            GET방식은 어디로 갈지 주소 + 어떤 데이터를 보낼지 같이 표시됨. 주소를 만들때 데이터 첨부까지 같이 진행.
        ...
        fun getRequestTopicDetail(
            context: Context, topicId : Int, handler: JsonResponseHandler?
        ) {
...
//            urlBuilder.addEncodedQueryParameter("value", inputVal)
        ...
        fun postRequestReply(
            context: Context, topicId: Int, content : String , handler: JsonResponseHandler?
        ) {
            val client = OkHttpClient()
        ...
        fun postRequestReplyLiek(
            context: Context, replyId : Int , islike : Boolean , handler: JsonResponseHandler?
        ) {
            val client = OkHttpClient()
